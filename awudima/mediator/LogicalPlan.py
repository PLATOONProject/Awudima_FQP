...
import abc
from awudima.pysparql import Service, Triple, Query
from awudima.pysparql import SPARQL
...
class Leaf(Tree):
    def __init__(self, s: Service, vs, dc, filter=None):
        if filter is None:
            filter = []
        self.vars = vs
        self.dict = dc
    ...
    def __hash__(self):
        return hash((self.vars, self.dict, self.size, self.degree(), self.service))
    ...
    def instantiate(self, d):
        ...
        for c in d:
            if c in newdict:
                del newdict[c]
        return Leaf(self.service.instantiate(d), newvars, newdict, filter=self.filters)
    ...
    def instantiateFilter(self, d, filter_str):
        ...
        for c in d:
            if c in newdict:
                del newdict[c]
        return Leaf(self.service.instantiateFilter(d, filter_str), newvars, newdict, filter=self.filters)
    ...
    def translate_to_sql(self, query, config):
        from awudima.sql import SQLTranslator

        translator = SQLTranslator(self.service, SPARQL.getPrefs(query.prefs), config)
        sql_query, variables, constants, res_temp = translator.translate()

        self.service.translated_query = str(pformat(sql_query))
        return self.service.endpoint, sql_query, set(variables), set(constants), res_temp

    def translate_to_mongo_ld_flat(self, query, config):
        from awudima.mongoql import MongoLDFlatTranslator

        translator = MongoLDFlatTranslator(self.service, SPARQL.getPrefs(query.prefs), config)
        mongo_query, variables, constants, res_temp = translator.translate()
        # print("********************************************************")
        ...
        # print(res_temp)
        # print('**************************************************')
        self.service.translated_query = str(pformat(mongo_query['pipeline'], indent=1)) if 'pipeline' in mongo_query else "None"
        self.service.endpoint = str(mongo_query['db']) + '.' + str(mongo_query['collection']) \
                                + ':ds-' + str(self.service.endpoint)

        return self.service.endpoint, mongo_query, set(variables), set(constants), res_temp

    def getInfoIO(self, query, config):
        from awudima.pyrml import DataSourceType
        if self.service.datasource.dstype == DataSourceType.MYSQL:
            return self.translate_to_sql(query, config)

        if self.service.datasource.dstype == DataSourceType.MONGODB_LD_FLAT:
            return self.translate_to_mongo_ld_flat(query, config)
    ...
    def getCount(self, query: Query, vars, endpointType):
        subquery = self.service.getTriples()
        if len(vars) == 0:
    ...
    while not(lss == []):
        ...
        for i in range(len(lss)):
            ...
            if isinstance(lss[i], Service) and isinstance(lss[m], Service) and \
                ...
                if len(lss[i].triples) > len(lss[m].triples):
                    m = i
                if len(get_so_variables(lss[i].triples)) < len(get_so_variables(lss[m].triples)):
                    m = i
        # Ordering between two leaf operators, i.e., SSQs,
