...
from awudima.operators.Join import Join
from awudima.operators.GJOperatorStructures import NHJTable, NHJPartition, NHJRecord, NHJFileDescriptor, isOverlapped


class XJoin(Join):

    def __init__(self, vars):
        self.left_table  = NHJTable()
        self.right_table = NHJTable()
        self.qresults    = Queue()
        self.vars        = vars
        self.timestamp   = 0
    ...
    def stage1(self, tuple, input, table1, table2):
        ...
        record = NHJRecord(tuple, self.timestamp, float("inf"))
...
#                    break
    ...
    def flushPartition(self):
        ...
        if (victim < self.left_table.size):
            ...
            self.left_table.partitions[victim] = NHJPartition()
            ...
            self.right_table.partitions[victim] = NHJPartition()

        # Update file descriptor
        if (victim in file_descriptor):
            lenfile = file_descriptor[victim].size
            file = open(file_descriptor[victim].file.name, 'a')
            file_descriptor.update({victim: NHJFileDescriptor(file, len(partition_to_flush.records) + lenfile, file_descriptor[victim].timestamps)})
        else:
            file = NamedTemporaryFile(suffix=".ht", prefix="", delete=False)
            file_descriptor.update({victim: NHJFileDescriptor(file, len(partition_to_flush.records), set())})
